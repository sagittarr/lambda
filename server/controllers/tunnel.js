...
//   failedRequests += 1;
//   return {ticker:task.ticker, err: 'error'}
// });

// queue.define("apiRequest", function (task) {
//   return Promise.try(function () {
//     return bhttp.get(q.compositeAlphaVantageURL(task.ticker), { decodeJSON: true });
//     // return bhttp.get(compositeLYURL(task.ticker, task.startDate, task.endDate), { decodeJSON: true });
//   }).then(function (response) {
//     return q.processDatafromAV(response.body, task.isBenchMark, task.startDate, task.endDate, task.pid, task.portfolio);
//   });
// }, {
//     concurrency: 50
//   });
...
function onMessage (tunnelId, type, content) {
    ...
    //     benchmark: undefined,
    //     counter: content.tickers.length
    //   }
    // for(var i =0 ; i< content.tickers.length; i++){
    //   var ticker = content.tickers[i]
    ...
    //     ticker = tokens[1];
    //   }
    //   var task = { ticker: ticker, isBenchMark: isBenchMark, startDate: content.startDate, endDate:content.endDate, portfolio: portfolio}
    //   console.log(task.portfolio.counter);      
    //   Promise.try(function () {
    //     /* The following queues up the actual task. Note how it returns a Promise! */
    //     return queue.push("apiRequest", task);
    //   }).then(function (stockdata) {
    //     q.aggregate(stockdata, function (p) { $broadcast('speak', p)})
    //   })
    // }
    // aggregate(stockdata)
    // Promise.try(function () {
    //   /* The following queues up the actual task. Note how it returns a Promise! */
    //   return queue.push("apiRequest", content);
    // }).then(function (stockdata) {
    //    $broadcast('speak', jsonResponse)
    // })
}
