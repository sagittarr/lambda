...
var config = {
    ...
    colors: ['#7cb5ec', '#f7a35c', '#434348', '#90ed7d', '#f15c80', '#8085e9', 'brown'],
...
function getSeriesDataItem(series, index) {
    ...
    series.forEach(function (item) {
        if (item.data[index] !== null && typeof item.data[index] !== 'undefinded') {
            ...
            seriesItem.data = item.desp? item.desp[index] :(item.format ? item.format(item.data[index]) : item.data[index]);
...
function drawSplitLines(points, color, context, opts, config) {
  points.forEach(function (item, index) {
    // console.log(item)
    drawToolTipSplitLine(item[0].x, opts, config, context, 4)
  })
};
...
function drawToolTipSplitLine(offsetX, opts, config, context, lineWidth = 1) {
    var startY = config.padding;
    var endY = opts.height - config.padding - config.xAxisHeight - config.legendHeight;
    // console.log(offsetX, startY, endY)
    context.beginPath();
    context.setStrokeStyle('darkgrey');
...
function drawLineDataPoints(series, opts, config, context) {
  // console.log
    ...
    series.forEach(function (eachSeries, seriesIndex) {
        ...
        calPoints.push(points);
        var splitPointList = splitPoints(points);
        if(eachSeries.drawSplitLines== true){
          drawSplitLines(splitPointList, eachSeries.splitLineColor, context, opts, config)
        }
        if(eachSeries.connect==true){
...
function drawRadarDataPoints(series, opts, config, context) {
    ...
    context.beginPath();
    context.setLineWidth(1);
    context.setStrokeStyle(radarOption.gridColor || "silver");
